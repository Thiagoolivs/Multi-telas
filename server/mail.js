 ...
 *
 * MAIL_FROM define o remetente ("Vistra <no-reply@seu-dominio.com>"). Sem
 * domínio verificado, os providers só entregam para o seu próprio e-mail.
 */
const RESEND_KEY = process.env.RESEND_API_KEY || '';
const BREVO_KEY = process.env.BREVO_API_KEY || '';
const FROM = process.env.MAIL_FROM || 'Vistra <onboarding@resend.dev>';
...
function parseFrom(s) {
  ...
  return m ? { name: m[1] || 'Vistra', email: m[2] } : { name: 'Vistra', email: String(s).trim() };
}
...
function resetEmail(link) {
  const text = 'Você pediu para redefinir sua senha no Vistra.\n\nAbra este link (vale por 1 hora):\n' + link +
  ...
  <p style="font-size:14px;line-height:1.6;margin:0 0 20px;color:#334155">Você pediu para redefinir sua senha no Vistra. O link vale por 1 hora.</p>
  ...
  return { subject: 'Redefinir sua senha · Vistra', html, text };
}

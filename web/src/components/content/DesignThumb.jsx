    ...
    <div className={'relative overflow-hidden rounded-md border border-line ' + (fitHeight ? '' : 'w-full')} style={{ ...ar, ...bg }}>
      {els.map((e, i) => (
        <div key={i} style={{ position: 'absolute', left: e.x + '%', top: e.y + '%', width: e.w + '%', height: e.h + '%', transform: `rotate(${e.rot || 0}deg)`, overflow: 'hidden' }}>
          {e.tipo === 'texto'
            ? <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: e.cor, fontWeight: e.peso, fontSize: 'clamp(6px,' + textFontCqw(e, formato) + 'cqw,60px)', textAlign: e.align, lineHeight: 1.05 }}>{e.text}</div>
            : e.tipo === 'icone'
